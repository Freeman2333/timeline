...
### HTML
*  используется PostHTML для трансформации HTML с помощью JS-плагинов
* для импорта блока в HTML файл ```<include src="footer/footer.html"></include>```
* для импорта модуля ```<module href="/footer/footer.html">Title</module>```
* В модуль можно вставлять другие компоненты, для этого на странице создания модуля нужно вставить тег <content>, пример: ```<header><content>(тут будет появляться контент)</content></header>```
...
* Используем фичи CSS4 c помощью postcss-preset-env
